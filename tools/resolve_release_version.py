...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Resolve the next Office Tools semantic release version from Git tags."
    )
    ...
    version = version_text(next_version)
    tag = f"v{version}"

    print(f"Previous version: {version_text(current) if current else 'none'}")
