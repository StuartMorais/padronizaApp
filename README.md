...
See `docs/ARCHITECTURE.md` for dependency rules and the document pipeline; `docs/UI_UX_GUIDED_CREATION_2026-08.md` for the guided Novo/Editar authoring and review UX; `docs/SCANNER_V6_SEMANTIC_2026-08.md` for the current local semantic/dynamic-region architecture; `docs/SCANNER_EDITABLE_INTENT_SAFETY_NET_2026-08.md` for the general recall-oriented field-discovery fallback; `docs/SCANNER_VISUAL_SAFETY_NET_2026-08.md` for the visual/red signal layer; `docs/SCANNER_V5_2026-08.md` and `docs/SCANNER_V4_2026-08.md` for the review-first and structure-first foundations; `docs/STRUCTURAL_TABLE_INTELLIGENCE_2026-08.md` for Word-table preservation; and `docs/DOCM_AND_RELEASE_2026-08.md` for macro-safe DOCM ingestion and automatic GitHub Releases.
...
- **Editable-intent discovery is not color-dependent:** after stronger structural/semantic detectors have had first ownership, a final review-only safety net can surface unfamiliar black `Rótulo: valor` pairs, adjacent table values, compact black choice lists, explicit `[INFORMAR ...]`/`<Descrever ...>` placeholders, highlighted/underlined/non-red styled spans, and fields inside Word text boxes/shapes. Red formatting remains one signal, not a prerequisite.
...
- In candidate review, **Campo** is always a human-facing field name and **Trecho no documento** is a readable excerpt from the real source region; raw IDs, detector names, Python list representations, and other implementation details stay behind `Detalhes técnicos`. Generic detections try nearby labels/semantic suggestions before falling back to a humanized identifier.
