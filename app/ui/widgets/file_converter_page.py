...
class _ConversionTask(QRunnable):
    ...
    def run(self) -> None:
        ...
        try:
            with staged_output(self.destination, suffix=self.destination.suffix) as staged:
                if self.direction == "docx_to_pdf":
                    DEFAULT_CONVERTER.docx_to_pdf(
                        self.source,
                        staged,
                        warnings=warnings,
                ...
                publish_staged_output(staged, self.destination)
            ...
            self.signals.finished.emit(
                ...
                str(self.source),
                str(self.destination),
                f"Erro inesperado de conversão ({report.error_id}): {exc}",
                "",
            )
            return

        backend = (
            DEFAULT_CONVERTER.last_backend()
            if self.direction == "docx_to_pdf"
            else "Conversor PDF → DOCX integrado"
        )
        self.signals.finished.emit(
            True,
            self.direction,
...
class FileConverterPage(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._active_task: _ConversionTask | None = None
    ...
    def _cancel_conversion(self) -> None:
        ...
        self.status_title.setText('Cancelando conversão')
        self.status_detail.setText('Aguardando o conversor interromper com segurança…')
    ...
    def _conversion_finished(
        ...
        if not success and backend == "__cancelled__":
            self._active_destination = None
            self._show_ready_state(detail='Conversão cancelada. O arquivo original não foi alterado.')
            return
        ...
        self.status_detail.setText(
            f"Criado: {self._last_output.name}{backend_suffix}"
        )
