...
class TutorialPage(QWidget):
    ...
    def _markers_tab(self) -> QScrollArea:
        ...
        layout.addWidget(
            self._marker_example_card(
                'Texto com valor padrão',
                '{{default_or_text:contratacao.detalhamento|Não há detalhamento adicional.}}',
                'Usa o texto informado após | quando o usuário deixa o campo vazio; se houver preenchimento, insere o texto do usuário.',
            )
        )
        ...
        layout.addWidget(
            self._guide_group(
                ...
                [
                    ...
                    ('Prefixos especiais', 'Use date:, checkbox:, dropdown:, single_choice: e default_or_text:. repeat: é usado somente em linhas de tabelas repetíveis. Outros tipos são escolhidos no Editor de Modelo.'),
                ],
            )
        )
    ...
    def _field_types_group(self) -> QGroupBox:
        ...
        rows = [
            ...
            ('Texto com valor padrão', '{{default_or_text:contratacao.detalhamento|Não há detalhamento adicional.}}', 'Mantém um texto padrão quando o campo fica vazio e o substitui quando o usuário informa outro conteúdo.'),
