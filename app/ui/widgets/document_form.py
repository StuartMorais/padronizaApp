...
from app.domain.field_metadata import dropdown_option_values, is_assisted_detection_field
from app.domain.field_handlers import field_handler
from app.domain.validation import infer_field_type, sample_value, validate_field, validation_hint
from app.document.understanding.layout import group_form_grid_static_rows, layout_blocks
from app.ui.widgets.field_container import FieldContainer
...
class DocumentForm(QWidget):
    ...
    def _build_choice_block(
        ...
        if fields:
            ...
            if correction is not None:
                ...
                correction_row.addWidget(correction)
    ...
    def _create_field_card(
        ...
        *,
        ...
        help_title = (
            str(field.get("help_title", label.rstrip(" *"))).strip()
            or label.rstrip(" *")
        )
        ...
        card = FieldContainer(
            ...
            help_text=help_text,
            hint_text=hint_text,
            assisted_detection=is_assisted_detection_field(field),
            suppress_label=suppress_label,
        )
        card.edit_field_requested.connect(self.edit_field_requested.emit)

        self.field_error_labels[field_id] = card.error_label
        if hint_text:
            self.field_hint_labels[field_id] = card.hint_label
        return card
    ...
    def _should_span_full_width(field_id: str, field_type: str, field: dict[str, Any]) -> bool:
        ...
        if layout == "full_width" or bool(field.get("full_width", False)):
            return True
        if field_handler(field_type).full_width:
            return True
        if field_type == "dropdown":
    ...
    def _placeholder_for(field_id: str, field_type: str) -> str:
        registered = field_handler(field_type).placeholder
        if registered:
            return registered
        normalized = field_id.casefold()
